...
bool binary_exists_for_current_device()
{
  ...
  // clear the MUSA global error state if we just set it, so that
  // check_musa_error doesn't complain
...
void list_devices(void)
{
  ...
  if(deviceCount == 0)
  {
    std::cout << "There is no device supporting musa" << std::endl;
  }
  ...
  for (int dev = 0; dev < deviceCount; ++dev)
  {
    ...
    if(dev == 0)
    {
      if(deviceProp.major == 9999 && deviceProp.minor == 9999)
        ...
        std::cout << "There is 1 device supporting musa" << std:: endl;
      else
        std::cout << "There are " << deviceCount <<  " devices supporting musa" << std:: endl;
    }
...
std::vector<int> MUSATestDriver::target_devices(const ArgumentMap &kwargs)
{
...
bool MUSATestDriver::check_musa_error(bool concise)
{
  ...
  if(musaSuccess != error)
  {
    if(!concise)
    {
      std::cout << "[ERROR] musa error detected before running tests: ["
...
bool MUSATestDriver::post_test_smoke_check(const UnitTest &test, bool concise)
{
  ...
  if(musaSuccess != error)
  {
    if(!concise)
    {
      std::cout << "\t[ERROR] musa error detected after running " << test.name << ": ["
                << std::string(musaGetErrorName(error))
                << ": "
                << std::string(musaGetErrorString(error))
                << "]" << std::endl;
...
bool MUSATestDriver::run_tests(const ArgumentSet &args, const ArgumentMap &kwargs)
{
  ...
  if(check_musa_error(concise)) return false;
...
int MUSATestDriver::current_device_architecture() const
{
  int current = -1;
...
{
  static MUSATestDriver s_instance;
  return s_instance;
}
