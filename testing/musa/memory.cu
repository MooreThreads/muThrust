...
void TestSelectSystemCudaToCpp()
{
  ...
  thrust::musa_cub::cross_system<thrust::musa::tag,thrust::cpp::tag> musa_to_cpp(musa_tag, cpp_tag);

  // select_system(cuda::tag, thrust::host_system_tag) should return musa_to_cpp
  bool is_musa_to_cpp = are_same_type(musa_to_cpp, select_system(musa_tag, cpp_tag));
  ASSERT_EQUAL(true, is_musa_to_cpp);
}
