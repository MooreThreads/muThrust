...
// depending on the heavyweight thrust/system/musa/memory.h header
...
void *malloc(execution_policy<DerivedPolicy> &, std::size_t n)
{
  ...
  if (THRUST_IS_HOST_CODE) {
        ...
        musaError_t status = alloc.DeviceAllocate(&result, n);
      #else
        musaError_t status = musaMalloc(&result, n);
      #endif
...
void free(execution_policy<DerivedPolicy> &, Pointer ptr)
{
  if (THRUST_IS_HOST_CODE) {
      ...
      #ifdef __CUB_CACHING_MALLOC
        cub::CachingDeviceAllocator &alloc = get_allocator();
        musaError_t status = alloc.DeviceFree(thrust::raw_pointer_cast(ptr));
      #else
        musaError_t status = musaFree(thrust::raw_pointer_cast(ptr));
      #endif
      musa_cub::throw_on_error(status, "device free failed");
    #endif
  } else {
    #if THRUST_INCLUDE_DEVICE_CODE
