...
struct marker_deleter final
{
  ...
  {
    if (nullptr != e)
      thrust::musa_cub::throw_on_error(musaEventDestroy(e));
  }
};
...
public:
  ...
  bool ready() const
  {
    ...
    thrust::musa_cub::throw_on_error(err);
  ...
  void wait() const
  {
    thrust::musa_cub::throw_on_error(musaEventSynchronize(handle_.get()));
  }
...
public:
  ...
  {
    thrust::musa_cub::throw_on_error(musaEventRecord(e.get(), handle_.get()));
  }
...
// Precondition: `device` is the current MUSA device.
...
public:
  ...
  {
    thrust::musa_cub::throw_on_error(musaGetDevice(&device_));
  }
...
{
  int device_id = 0;
  thrust::musa_cub::throw_on_error(musaGetDevice(&device_id));
