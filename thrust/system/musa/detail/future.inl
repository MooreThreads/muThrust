...
#include <thrust/system/musa/future.h>
...
{
  __host__
  void operator()(MUevent_st* e) const
  {
    if (nullptr != e)
      thrust::cuda_cub::throw_on_error(musaEventDestroy(e));
  }
};
...
{
  using native_handle_type = MUevent_st*;

private:
  std::unique_ptr<MUevent_st, marker_deleter> handle_;
...
public:
  ...
  bool ready() const
  {
    musaError_t const err = musaEventQuery(handle_.get());
  ...
  void wait() const
  {
    thrust::cuda_cub::throw_on_error(musaEventSynchronize(handle_.get()));
  }
...
{
  __host__
  void operator()(MUstream_st* s) const
  {
    if (nullptr != s)
      thrust::cuda_cub::throw_on_error(musaStreamDestroy(s));
  }
};
...
{
  using native_handle_type = MUstream_st*;

private:
  std::unique_ptr<MUstream_st, stream_conditional_deleter> handle_;
...
public:
  ...
  {
    thrust::cuda_cub::throw_on_error(
      musaStreamSynchronize(handle_.get())
    );
  }
  ...
  {
    thrust::cuda_cub::throw_on_error(
      musaStreamWaitEvent(handle_.get(), e.get(), 0)
    );
  }
  ...
  {
    thrust::cuda_cub::throw_on_error(musaEventRecord(e.get(), handle_.get()));
  }
...
public:
  ...
  friend __host__
  unique_eager_future_promise_pair<X, XPointer>
  thrust::system::musa::detail::make_dependent_future(
...
public:
  ...
  {
    thrust::cuda_cub::throw_on_error(musaGetDevice(&device_));
  }
  ...
  thrust::system::musa::detail::try_acquire_stream(
  ...
  friend __host__
  unique_eager_event
  thrust::system::musa::detail::make_dependent_event(
    std::tuple<Dependencies...>&& deps
  );
};
...
{
  int device_id = 0;
  thrust::cuda_cub::throw_on_error(musaGetDevice(&device_id));
