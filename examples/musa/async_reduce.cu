...
// is stored to a pointer to MUSA global memory. The calling thread waits for the result of the reduction to
// be ready by synchronizing with the MUSA stream on which the __global__ function is launched.
//
...
int main()
{
  ...
  // method 1: call thrust::reduce from an asynchronous MUSA kernel launch

  // create a MUSA stream
  musaStream_t s;
  musaStreamCreate(&s);

  // launch a MUSA kernel with only 1 thread on our stream
